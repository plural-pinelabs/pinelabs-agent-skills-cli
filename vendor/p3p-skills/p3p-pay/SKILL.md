...
If install fails due to permissions:

```bash
sudo npm install -g @pine-labs-online/p3p-cli
```
...
```bash
echo $PINE_LABS_CLIENT_ID
echo $PINE_LABS_CLIENT_SECRET
echo $PINE_LABS_ENV        # SANDBOX or PRODUCTION
```
...
p3p listen --print-json                                          # print full event JSON to console
...
| Flag | Description |
|---|---|
| `--api-key <client_id:client_secret>` | Override credentials inline |
